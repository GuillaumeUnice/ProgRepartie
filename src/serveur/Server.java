...
public class Server {
	...
	List<String> receiveReq(String inputLine, BufferedReader in)
			throws IOException {
	...
	Object lancerMethode(Object obj, List<String> res, String nomMethode)
			throws Exception {
	...
	public static void main(String[] args) throws Throwable {
		...
		try {
			...
			System.err.println("Le serveur ne peut écouter sur le port : "
					+ port);
			System.exit(1);
		}
		...
		while (!(inputLine = in.readLine()).equals(Application.END_APP)) {
			...
			try {
				res = (LinkedList<String>) server.lancerMethode(
						server.getModel(), res, res.pop());
			} catch (InvocationTargetException e) {
				try {
					throw e.getCause();
				} catch (ApplicationException err) {
					res = Application.appError(err.msg());
				}
			}
			// LinkedList<String> ret =
			// server.model.listNickNameByName("Julien");
